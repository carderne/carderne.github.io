...
[Remix](https://remix.run/) pushed hard on forms and backend routing. Hit a form, reload. Next.js noticed this and saw that it was good, so they created the App router and a nightmare transition for their users, but the dust is now settling (at least for greenfield projects). And now all this stuff has found its way into React itself, and so now traditional forms are being blessed, along with [React Server Components](https://react.dev/reference/rsc/server-components).

But they're new and the best ways of using them don't seem to be obvious. Specifically:
1. Data validation
2. Handling errors
3. Maintaining state
4. Optimistic loading

So I played around a bit and came up with what I think is a pretty good pattern for fancy React 19 forms. As always, you can just skip to the repo [carderne/react-forms](https://github.com/carderne/react-forms) (or scroll to the bottom) if you prefer. I'm going to assume you're already familiar with with Server Components and `"use server"` and forms in general. If not, it's worth reading [the Next.js docs on the topic](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations) and following the interesting links where they lead.
